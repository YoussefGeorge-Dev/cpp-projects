# 💻 Introduction to Programming Labs

<div align="center">

![C++](https://img.shields.io/badge/C++-17-black?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Platform](https://img.shields.io/badge/Platform-Windows%20%7C%20Linux%20%7C%20macOS-000000?style=for-the-badge)

**C++ lab solutions and coursework from The British University in Egypt (BUE)**

</div>

---

## 🚀 How to Run

### Visual Studio Code

1. Install the [C/C++ Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-extension-pack)
2. Open any `.cpp` file
3. Click the **▷ Run** button

### Command Line

```bash
g++ Lab04.cpp -o output && ./output
```

---

## 📚 Labs

Each lab is refactored into a **function menu** format: every question is a standalone function, and `main()` presents a terminal menu so you can run any question on demand.

| Lab       | Topics                                                  | File                            |
| :-------- | :------------------------------------------------------- | :------------------------------- |
| **Lab01** | Nested If · Switch-Case · Nested Loops                    | [Lab01.cpp](./Lab01/Lab01.cpp)   |
| **Lab02** | One-Dimensional Arrays                                     | [Lab02.cpp](./Lab02/Lab02.cpp)   |
| **Lab03** | Two-Dimensional Arrays · Matrix Operations                 | [Lab03.cpp](./Lab03/Lab03.cpp)   |
| **Lab04** | Functions Part 1 · Parameters · Return Values              | [Lab04.cpp](./Lab04/Lab04.cpp)   |
| **Lab05** | Pointer Basics · Arrays with Pointers · Swapping           | [Lab05.cpp](./Lab05/Lab05.cpp)   |
| **Lab06** | Call by Reference · Arrays · Matrix Functions               | [Lab06.cpp](./Lab06/Lab06.cpp)   |
| **Lab07** | Structures · Enums · Data Modeling                          | [Lab07.cpp](./Lab07/Lab07.cpp)   |
| **Lab08** | File Handling · Read/Write · seekg() · Text Processing      | [Lab08.cpp](./Lab08/Lab08.cpp)   |
| **Lab09** | Advanced File Handling · Exception Handling                 | [Lab09.cpp](./Lab09/Lab09.cpp)   |
| **Lab10** | Recursive Functions · Recursion with Arrays                 | [Lab10.cpp](./Lab10/Lab10.cpp)   |

---

## 📝 Notes

* All applications are terminal-based C++ programs.
* Labs 08 and 09 include file-handling exercises that read from `.txt` files (e.g. `Q1.txt`, `Q3_Inventory.txt`). These are included alongside their respective lab source files — keep them next to the compiled executable when running, since they're read via relative paths.

---

## 📚 References

Resources and references used throughout the labs:

* [cppreference](https://en.cppreference.com/)
* [learncpp](https://www.learncpp.com/)
* [cplusplus](https://cplusplus.com/)
* BUE course slides and lab sheets

---

> *"C makes it easy to shoot yourself in the foot; C++ makes it harder, but when you do it blows your whole leg off."* — Bjarne Stroustrup